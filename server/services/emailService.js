...
let maildevUnavailableLogged = false;
...
class EmailService {
  ...
  async sendWithRetry(options, maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // In local maildev mode, avoid noisy retries when maildev isn't running.
        if (
          (process.env.EMAIL_MODE || "maildev") === "maildev" &&
          error.message.includes("ECONNREFUSED")
        ) {
          if (!maildevUnavailableLogged) {
            console.warn(
              "⚠️ Maildev is not running (SMTP refused). Skipping email sends in development.",
            );
            console.warn(
              "💡 Start it with: docker compose -f docker-compose.dev.yml up maildev",
            );
            maildevUnavailableLogged = true;
          }
  ...
  async logEmailAttempt(data) {
    // Keep lightweight logging only; email events aren't tied to a single actor.
    console.log("Email attempt:", data);
  }
